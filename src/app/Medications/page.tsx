...
export default function Medications() {
  ...
  const [medArr, setMedArr] = useState<{ frequency_h_offset: number, medication_id: number, name: string, description: string, dosage: string, useNow: boolean}[]>([]);
  ...
  useEffect(() => {
      (async () => {
        const response = await fetch("/api/medication/ComingUp", {
        method: "POST",
        headers: {
      ...
      if (JSON.stringify(tmp) !== "{}")
        tmp.sort((a: any, b: any) => a.frequency_h_offset - b.frequency_h_offset);
      else tmp = [];
  ...
  const medicationClick = async (index: number) => {
    if (!medArr[index].useNow) {
      return  
    }

    const medicationIndex = medArr[index].medication_id;


    const newArr = [...medArr];
    const [item] = newArr.splice(index, 1); // remove the item at index
    newArr.push(item);                      // add it to the end
    setMedArr(newArr);
    ...
    const data = fetch("/api/medication/use", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({id: medicationIndex})
    });
  };
  ...
  function formatTime(targetHour: number): string {
    ...
    if (hours === 0 && minutes <= 15) return "Now"
    ...
    <div className={`${AnonymousPro.className} font-bold w-screen h-screen relative`}>
      ...
      <NavBar>
        <main className={`${AnonymousPro.className} w-full h-full`}>
          ...
          <h1 className='text-navy-blue mb-2 ml-20 text-5xl'>Next up</h1>
          <div className='flex flex-col items-center w-full h-3/6'>
            ...
            <div className=' w-11/12 h-full flex flex-row'>
              {medArr.map((med: {frequency_h_offset: number, name: string, useNow: boolean}, index) => (
              ...
              */  
              ...
              <div key={index} className='w-full h-full flex flex-col items-center'> 
                <h1 className='text-4xl'>{formatTime(med.frequency_h_offset)}</h1>
...
{ /* Each element should have a randomized "light and colorfull" color with a foreground appropriate border color 
  * Can only show a maximum of 3 elements the rest are just saved */}
                <div className={`
                  ...
                  items-center
                  justify-around
                  ${med.useNow ? "bg-blue-300 border-navy-blue": "bg-gray-400 border-gray-700"}
                `}
                ...
                >
                  ...
                  <div className='flex flex-col'>
                    ...
                    <h1 className={` ${med.useNow?  "bg-navy-blue": "bg-gray-500"} p-2 rounded-2xl text-white text-4xl`}>{med.name}</h1>
